...
if ($result->num_rows > 0) {
    ...
    if ($page > 1) {
        echo '<a href="#" onclick="fetchCommerceData('.($page - 1).'); event.preventDefault();">Previous</a>';
    }
    for ($i = 1; $i <= $total_pages; $i++) {
        echo '<a href="#" onclick="fetchCommerceData('.$i.');  event.preventDefault();">' . $i . '</a>';
    }
    if ($page < $total_pages) {
        echo '<a href="#" onclick="fetchCommerceData('.($page + 1).'); event.preventDefault();">Next</a>';
    }
    echo '</div>';
} else {
    echo "0 result";
